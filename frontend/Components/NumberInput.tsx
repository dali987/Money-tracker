...
const NumberInput = ({ className, name, disabled } : {className?: string, name?: string, disabled?: boolean}) => {
    const [number, setNumber] = useState(0);

    return (
        <div className={`relative min-w-45 ${className}`}>
            <button
            ...
            <input
                ...
                className="w-full input rounded-md transition focus:outline-offset-0 focus:outline-1 shadow-sm focus:shadow-lg appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
            <button
